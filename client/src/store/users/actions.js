...
export const fetchItems = () => ({
  ...
  shouldCallAPI: (state) => {
    ...
    // if user users are currently loading don't call again
    if (isLoading) return false;
    const isCached = Date.now() - loadedAt < CACHE_TIME;
    // if we don't have the user user or it's beyond the cache timeout make the api call
    return !loadedAt || !isCached;
  },
});

export const createItem = (user) => {
